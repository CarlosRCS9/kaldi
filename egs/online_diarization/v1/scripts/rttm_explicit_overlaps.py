    ...
    for recording_id in recordings_complex_segments:
      for segment in recordings_complex_segments[recording_id]:
        if len(segment.speakers) > 1:
          ...
          if recording_id not in non_overlap_segments:
            non_overlap_segments[recording_id] = []
          non_overlap_segments[recording_id].append(segment)
          non_overlap_duration += segment.duration
    print('non overlap time:', non_overlap_duration)
    print('overlap time:', overlap_duration)
    print('overlap time / non overlap time:', overlap_duration / (overlap_duration + non_overlap_duration))
  else:
