  ...
  for recording_id in sorted(list(recordings_segments.keys())):
    ...
    while sum(options_lengths) > 0:
      ...
      if option_index == 0:
        ...
        command.append('|sox ' + recording_filepath + ' -t ' + recording_extension + ' - trim ' + str(recordings_segments_last_ending) + ' ' + str(segment.duration))
        recordings_segments_last_ending = segment.ending
        recording_segments_index += 1
      else:
        segment = option
        command.append('|sox ' + segment.filepath + ' -t ' + segment.filepath.split('.')[-1] + ' - trim ' + str(segment.begining) + ' ' + str(segment.duration))
        last_ending = new_recording_segments[-1].ending if len(new_recording_segments) > 0 else 0
