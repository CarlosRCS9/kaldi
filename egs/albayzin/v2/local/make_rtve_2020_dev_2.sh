...
cat $data_dir/rttm/*ENVIRONMENT* \
  ...
  > $output_dir/environment_original.rttm
sed "/unknown/d" -i $output_dir/environment_original.rttm
sed -e "s/$/ <NA>/" -i $output_dir/environment_original.rttm
sort -k4,4 -n $output_dir/environment_original.rttm > $output_dir/environment_original_tmp.rttm
mv $output_dir/environment_original_tmp.rttm $output_dir/environment_original.rttm
# At this point the environment_original.rttm file has the standard sctructure
cat $output_dir/environment_original.rttm \
  | python3 scripts/rttm_explicit_overlap.py \
  > $output_dir/environment.rttm
# At this point the environment.rttm file has the same data as the environment_original.rttm, but with explicit overlap
...
cat $data_dir/rttm/*PLACE* \
  ...
  > $output_dir/place_original.rttm
sed "/unknown/d" -i $output_dir/place_original.rttm
sed -e "s/$/ <NA>/" -i $output_dir/place_original.rttm
sort -k4,4 -n $output_dir/place_original.rttm > $output_dir/place_original_tmp.rttm
mv $output_dir/place_original_tmp.rttm $output_dir/place_original.rttm
# At this point the place_original.rttm file has the standard sctructure
cat $output_dir/place_original.rttm \
  | python3 scripts/rttm_explicit_overlap.py \
  > $output_dir/place.rttm
# At this point the place.rttm file has the same data as the place_original.rttm, but with explicit overlap
...
cat $data_dir/rttm/*SEX* \
  ...
  > $output_dir/sex_original.rttm
sed "/unknown/d" -i $output_dir/sex_original.rttm
sed -e "s/$/ <NA>/" -i $output_dir/sex_original.rttm
sort -k4,4 -n $output_dir/sex_original.rttm > $output_dir/sex_original_tmp.rttm
mv $output_dir/sex_original_tmp.rttm $output_dir/sex_original.rttm
# At this point the sex_original.rttm file has the standard sctructure
cat $output_dir/sex_original.rttm \
  | python3 scripts/rttm_explicit_overlap.py \
  > $output_dir/sex.rttm
# At this point the sex.rttm file has the same data as the sex_original.rttm, but with explicit overlap
...
if [ $speaker_rename = true ]; then
  ...
  cat $output_dir/environment.rttm | python3 scripts/rename.py $output_dir/recording_name > $output_dir/environment_tmp.rttm
  mv $output_dir/environment_tmp.rttm $output_dir/environment.rttm
  cat $output_dir/place.rttm | python3 scripts/rename.py $output_dir/recording_name > $output_dir/place_tmp.rttm
  mv $output_dir/place_tmp.rttm $output_dir/place.rttm
  cat $output_dir/sex.rttm | python3 scripts/rename.py $output_dir/recording_name > $output_dir/sex_tmp.rttm
