...
if [ $# -lt 3 ]; then
  echo "Usage: $0 <mode> <path-to-rtve_2018_dev2> <path-to-output> <speaker-overlap>"
  echo " e.g.: $0 eval /export/corpora5/RTVE/RTVE2018DB/dev2 data/rtve_2018_dev2 true"
  exit 1
fi
...
# At this point the ref.rttm file has the standard sctructure, with no music and unknown
#  cat $output_dir/ref.rttm | python3 scripts/rttm_get_file_name.py rtve_2018 > $output_dir/recording_name
...
#cat $output_dir/ref.rttm | python3 scripts/rttm_get_speaker_name.py rtve_2018_speaker > $output_dir/speaker_name
...
if [ $mode = "train" ]; then
  cat $output_dir/ref.rttm \
    ...
    #  | python3 scripts/rename.py $output_dir/speaker_name > $output_dir/segments
fi
...
if [ $mode = "train" ]; then
  cat $output_dir/ref.rttm \
    ...
    #  | python3 scripts/rename.py $output_dir/speaker_name > $output_dir/utt2spk
fi
