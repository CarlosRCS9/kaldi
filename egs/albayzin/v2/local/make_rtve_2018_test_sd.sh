...
if [ $# -lt 3 ]; then
  echo "Usage: $0 <mode> <path-to-rtve_2018_test> <path-to-output> <speaker-overlap> <speaker-rename>"
  echo " e.g.: $0 eval /export/corpora5/RTVE/RTVE2018DB/test data/rtve_2018_test_sd true false"
  exit 1
fi
...
# ------------------------- ref.rttm ------------------------- #
cat $data_dir/references/rttm/SD/EC*.rttm \
  $data_dir/references/rttm/SD/LA24H*.rttm \
  $data_dir/references/rttm/SD/LM*.rttm \
  $data_dir/references/rttm/SD/LT24HTer*.rttm \
...
if [ $speaker_rename = true ]; then
  cat $output_dir/ref.rttm | python3 scripts/rttm_get_file_name.py rtve_2018_test_sd > $output_dir/recording_name
...
if [ $speaker_rename = true ]; then
  cat $output_dir/ref.rttm | python3 scripts/rttm_get_speaker_name.py rtve_2018_test_sd_speaker > $output_dir/speaker_name
fi
...
for filepath in $(ls -d $data_dir/audio/SD/*.aac); do
  if [ $speaker_rename = true ]; then
