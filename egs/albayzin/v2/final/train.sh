...
if [ $stage -le 0 ]; then
  ...
  utils/combine_data.sh data/voxceleb${suffix} data/voxceleb1_train data/voxceleb2_train data/voxceleb2_test

  # <speaker-overlap> <speaker-rename>
  local/make_rtve_2018_dev2_2.sh train $rtve_root/RTVE2018DB/dev2 data/rtve_2018${suffix}_train false true
fi

if [ $stage -le 1 ]; then
  for name in voxceleb${suffix} rtve_2018${suffix}_train; do
    data_dir=data/$name
    steps/make_mfcc.sh \
      --cmd "$train_cmd" \
...
if [ $stage -le 2 ]; then
  data_dir=data/voxceleb_rtve_2018${suffix}
  output_dir=exp/xvectors_voxceleb_rtve_2018${suffix}

  utils/combine_data.sh \
    $data_dir \
    data/voxceleb${suffix}_seg \
    data/rtve_2018${suffix}_train_seg
