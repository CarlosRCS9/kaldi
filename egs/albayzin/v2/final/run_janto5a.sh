...
if [ $stage -le 0 ]; then
  ...
  rm -rf data/rtve_2018${suffix}
  local/make_rtve_2018_dev2.sh oracle $rtve_root/RTVE2018DB/dev2 data/rtve_2018${suffix} false
  rm -rf data/rtve_2018${suffix}_train
  local/make_rtve_2018_dev2.sh oracle $rtve_root/RTVE2018DB/dev2 data/rtve_2018${suffix}_train false
  rm -rf data/rtve_2020${suffix}
  local/make_rtve_2020_dev.sh oracle $rtve_root/RTVE2020DB/dev data/rtve_2020${suffix} false
fi

if [ $stage -le 1 ]; then
  # Make MFCCs for each dataset.
  for name in rtve_2018${suffix} rtve_2018${suffix}_train rtve_2020${suffix}; do
    steps/make_mfcc.sh \
