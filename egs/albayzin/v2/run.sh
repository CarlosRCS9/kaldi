...
if [ $stage -le 0 ]; then
  ...
  # Now prepare the VoxCeleb1 train and test data.
  ...
  # Prepare the RTVE 2018 development set.
  local/make_rtve_2018_dev2.sh $rtve_root/RTVE2018DB/dev2 data/rtve_2018 true
fi

if [ $stage -le 1 ]; then
  # Make MFCCs for each dataset.
  for name in train rtve_2018; do
    steps/make_mfcc.sh \
  ...
  sid/compute_vad_decision.sh \
    --vad-config conf/vad_zero.conf \
    --nj 40 \
    --cmd "$train_cmd" \
    data/rtve_2018 \
    exp/make_vad \
    $vaddir
  utils/fix_data_dir.sh data/rtve_2018
  ...
  echo "0.01" > data/rtve_2018_cmn/frame_shift
...
if [ $stage -le 2 ]; then
  utils/combine_data.sh data/train_rtve_2018 data/train data/rtve_2018

  frame_shift=0.01
  awk -v frame_shift=$frame_shift '{print $1, $2*frame_shift;}' data/train_rtve_2018/utt2num_frames > data/train_rtve_2018/reco2dur
  ...
  steps/data/reverberate_data_dir.py \
    ...
    data/train_rtve_2018 data/train_reverb
  cp data/train_rtve_2018/vad.scp data/train_reverb/
  ...
  steps/data/augment_data_dir.py --utt-suffix "noise" --fg-interval 1 --fg-snrs "15:10:5:0" --fg-noise-dir "data/musan_noise" data/train_rtve_2018 data/train_noise
  # Augment with musan_music
  steps/data/augment_data_dir.py --utt-suffix "music" --bg-snrs "15:10:8:5" --num-bg-noises "1" --bg-noise-dir "data/musan_music" data/train_rtve_2018 data/train_music
  # Augment with musan_speech
  steps/data/augment_data_dir.py --utt-suffix "babble" --bg-snrs "20:17:15:13" --num-bg-noises "3:4:5:6:7" --bg-noise-dir "data/musan_speech" data/train_rtve_2018 data/train_babble
...
if [ $stage -le 3 ]; then
  ...
  utils/combine_data.sh data/train_combined data/train_aug_1m data/train_rtve_2018
fi
...
# At this point the extractor model is trained

if [ $stage -le 9 ]; then
  # Prepare the RTVE 2018 development set for evaluation.
  local/make_rtve_2018_dev2.sh $rtve_root/RTVE2018DB/dev2 data/rtve_2018_eval
  local/make_rtve_2020_dev.sh $rtve_root/RTVE2020DB/dev data/rtve_2020_eval
fi

if [ $stage -le 10 ]; then
  # Make MFCCs for each dataset.
  for name in rtve_2018_eval rtve_2020_eval; do
    steps/make_mfcc.sh \
      --write-utt2num-frames true \
      --mfcc-config conf/mfcc.conf \
      --nj 40 \
  ...
  sid/compute_vad_decision.sh \
    ...
    exp/make_vad \
    $vaddir
  utils/fix_data_dir.sh data/rtve_2018_eval
  ...
  sid/compute_vad_decision.sh \
    ...
    exp/make_vad \
    $vaddir
  utils/fix_data_dir.sh data/rtve_2020_eval
  ...
  echo "0.01" > data/rtve_2018_eval_cmn/frame_shift
  echo "0.01" > data/rtve_2020_eval_cmn/frame_shift
  # Create segments to extract x-vectors from for data.
  # The segments are created using an energy-based speech activity
  # detection (SAD) system, but this is not necessary.  You can replace
  # this with segments computed from your favorite SAD.
...
if [ $stage -le 11 ]; then
  ...
  diarization/nnet3/xvector/extract_xvectors.sh --cmd "$train_cmd --mem 5G" \
    ...
    data/rtve_2018_eval_cmn $nnet_dir/xvectors_rtve_2018_eval

  diarization/nnet3/xvector/extract_xvectors.sh --cmd "$train_cmd --mem 5G" \
    --nj 40 --window 1.5 --period 0.25 --apply-cmn false \
    --min-segment 0.5 $nnet_dir \
    data/rtve_2020_eval_cmn $nnet_dir/xvectors_rtve_2020_eval
...
if [ $stage -le 12 ]; then
  ...
  "$train_cmd" $nnet_dir/xvectors_rtve_2018_eval/log/plda.log \
    ivector-compute-plda ark:$nnet_dir/xvectors_train_segmented_128k/spk2utt \
      ...
      | transform-vec $nnet_dir/xvectors_rtve_2018_eval/transform.mat ark:- ark:- \
      | ivector-normalize-length ark:- ark:- |" \
    $nnet_dir/xvectors_rtve_2018_eval/plda || exit 1;
fi
