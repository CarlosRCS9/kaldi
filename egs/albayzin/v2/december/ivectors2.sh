...
suffix=_20201204_EXP001
rtve_root=/export/corpora5/RTVE
mfcc_conf=../../dihard_2018/v1/conf/mfcc.conf
ivector_extractor_model=/export/b03/carlosc/repositories/kaldi/egs/dihard_2018/v1/exp/extractor_c2048_i400

stage=0

if [ $stage -le 0 ]; then
  local/make_rtve_2018_dev2_2.sh eval $rtve_root/RTVE2018DB/dev2 data/rtve_2018_dev2${suffix} false true
  local/make_rtve_2018_test_sd.sh eval $rtve_root/RTVE2018DB/test data/rtve_2018_test_sd${suffix} false true

  local/make_rtve_2020_dev_2.sh eval $rtve_root/RTVE2020DB/dev data/rtve_2020_dev${suffix} false true
  local/make_rtve_2020_test_sd.sh $rtve_root/RTVE2020DB/test/audio/SD data/rtve_2020_test_sd${suffix}
fi

if [ $stage -le 1 ]; then
  for name in rtve_2018_dev2${suffix} rtve_2018_test_sd${suffix} rtve_2020_dev${suffix} rtve_2020_test_sd${suffix}; do
    data_dir=data/$name
    steps/make_mfcc.sh \
  ...
  for name in rtve_2018_dev2${suffix} rtve_2018_test_sd${suffix} rtve_2020_dev${suffix} rtve_2020_test_sd${suffix}; do
    data_dir=data/$name
    cat $data_dir/utt2dur | awk '{print $1" "$1" 0.0 "$2}' > $data_dir/segments
  done

  for name in rtve_2018_dev2${suffix} rtve_2018_test_sd${suffix} rtve_2020_dev${suffix} rtve_2020_test_sd${suffix}; do
    data_dir=data/$name
    output_dir=data/${name}_cmn
    ...
    if [ -f $data_dir/vad.scp ]; then
      cp $data_dir/vad.scp $output_dir/
    fi
...
if [ $stage -le 2 ]; then
  for name in rtve_2018_dev2${suffix} rtve_2018_test_sd${suffix} rtve_2020_dev${suffix} rtve_2020_test_sd${suffix}; do
    data_dir=data/${name}_cmn
    output_dir=exp/${name}_ivectors
    diarization/extract_ivectors.sh \
      --cmd "$train_cmd --mem 20G" \
      --nj 9 \
      --window 1.5 \
      --period 0.75 \
      --apply-cmn false \
