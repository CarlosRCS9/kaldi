...
use vars qw ($opt_h $opt_w $opt_W $opt_d $opt_D $opt_R $opt_r $opt_S $opt_s $opt_l $opt_c $opt_x $opt_b);
...
use Getopt::Std;
getopts ('nhdDwWox1mbvec:R:r:S:s:t:T:g:p:P:a:A:u:l:y:Y:z:Z:M:');
...
#sort and check data
	...
	foreach $chnl (keys %{$data->{$file}}) {
		...
		if (($data_type eq "SPEAKER") or ($data_type eq "FACE")){
...
#map reference metadata times to ref words
	...
	next if ($md->{WDUR} > 0 or
		 $md->{TYPE} =~ /^(IP|CB)$/);
        next if (not $opt_W and not $opt_w and (($md->{TYPE} eq "SPEAKER") or ($md->{TYPE} eq "FACE")));
...
#	printf "  REF_SPK %s, SYS_SPK %s, SPKR_info %s, miss_name %s -- SYS_TYPE %s\n",$ref_spkr,$sys_spkr,$spkr_info->{SYS}{$sys_spkr}{TYPE},$miss_name,$sys_type;
...
#	printf "    SCORED_SPEAKER %f %f\n", $nref,$seg_dur;
	$stats->{MISSED_SPEAKER} += $seg_dur*max($nref-$nsys,0);
#	printf "    MISSED_SPEAKER %f %f\n", max($nref-$nsys,0),$seg_dur;
...
#overlapping speech computed from SPEAKER data
	...
	next unless ((($token->{TYPE} eq "SPEAKER") or ($token->{TYPE} eq "FACE")) and
		     $token->{TDUR} > 0);
...
#gather data needed to create noscore zones
	...
	elsif ((($token->{TYPE} eq "SPEAKER") or ($token->{TYPE} eq "FACE")) and
	       $token->{TDUR} > 0) {
...
sub uem_from_rttm {
	...
	($tbeg, $tend) = (min($tbeg,$token->{TBEG}), max($tend,$token->{TEND})) if
	    $token->{TYPE} =~ /^(SEGMENT|SPEAKER|FACE|SU|EDIT|FILLER|IP|CB|A\/P|LEXEME|NON-LEX)$/;
    }
...
#accumulate statistics
   ...
   if ($rttmfile_type eq "SPEAKER") {
    ...
    if ($rttmfile_type eq "FACE") {
    foreach my $condition (sort keys %cum_scores) {
	print_fd_scores ($condition, $cum_scores{$condition}) if $condition !~ /ALL/;
    }
    print_fd_scores ("ALL", $cum_scores{ALL});
    }
}
...
sub print_fd_scores {
    ...
    printf "\n*** Performance analysis for Face Diarization for $condition ***\n\n";

    printf "    EVAL TIME =%10.2f secs\n", $scores->{EVAL_TIME} ;
    printf "  EVAL VIDEO =%10.2f secs (%5.1f percent of evaluated time)\n", $scores->{EVAL_SPEECH},
        100*$scores->{EVAL_SPEECH}/$scores->{EVAL_TIME};
    printf "  SCORED TIME =%10.2f secs (%5.1f percent of evaluated time)\n",
        $scores->{SCORED_TIME}, 100*$scores->{SCORED_TIME}/$scores->{EVAL_TIME};
    printf "SCORED VIDEO =%10.2f secs (%5.1f percent of scored time)\n",
        $scores->{SCORED_SPEECH}, 100*$scores->{SCORED_SPEECH}/$scores->{SCORED_TIME};
    printf "   EVAL NAMES =%7d        \n", $scores->{EVAL_WORDS};
    printf " SCORED NAMES =%7d         (%5.1f percent of evaluated words)\n",
        $scores->{SCORED_WORDS}, 100*$scores->{SCORED_WORDS}/$scores->{EVAL_WORDS};
    print "---------------------------------------------\n";
    printf "MISSED VIDEO =%10.2f secs (%5.1f percent of scored time)\n",
        $scores->{MISSED_SPEECH}, 100*$scores->{MISSED_SPEECH}/$scores->{SCORED_TIME};
    printf "FALARM VIDEO =%10.2f secs (%5.1f percent of scored time)\n",
        $scores->{FALARM_SPEECH}, 100*$scores->{FALARM_SPEECH}/$scores->{SCORED_TIME};
    printf " MISSED NAMES =%7d         (%5.1f percent of scored words)\n",
        $scores->{MISSED_WORDS}, 100*$scores->{MISSED_WORDS}/$scores->{SCORED_WORDS};
    print "---------------------------------------------\n";
    printf "SCORED FACE TIME =%10.2f secs (%5.1f percent of scored video)\n",
        $scores->{SCORED_SPEAKER}, 100*$scores->{SCORED_SPEAKER}/$scores->{SCORED_SPEECH};
    printf "MISSED FACE TIME =%10.2f secs (%5.1f percent of scored face time)\n",
        $scores->{MISSED_SPEAKER}, 100*$scores->{MISSED_SPEAKER}/$scores->{SCORED_SPEAKER};
    printf "FALARM FACE TIME =%10.2f secs (%5.1f percent of scored face time)\n",
        $scores->{FALARM_SPEAKER}, 100*$scores->{FALARM_SPEAKER}/$scores->{SCORED_SPEAKER};
    printf " FACE ERROR TIME =%10.2f secs (%5.1f percent of scored face time)\n",
        $scores->{SPEAKER_ERROR}, 100*$scores->{SPEAKER_ERROR}/$scores->{SCORED_SPEAKER};
    printf "FACE ERROR WORDS =%7d         (%5.1f percent of scored face words)\n",
        $scores->{ERROR_WORDS}, 100*$scores->{ERROR_WORDS}/$scores->{SCORED_WORDS};
    print "---------------------------------------------\n";
#    if ($condition eq "ALL") {
#      printf " OVERALL SPEAKER DIARIZATION ERROR = %.2f percent of scored speaker time\n",
...
#	    $scores->{SCORED_SPEAKER};
#    } else {
      printf " OVERALL FACE DIARIZATION ERROR = %.2f percent of scored face time  %s\n",
         100*($scores->{MISSED_SPEAKER} + $scores->{FALARM_SPEAKER} + $scores->{SPEAKER_ERROR})/
    	    $scores->{SCORED_SPEAKER}, "`($condition)";
#    }
    print "---------------------------------------------\n";
    printf " Face type confusion matrix -- face weighted\n";
    summarize_speaker_type_performance ("NSPK", $scores->{TYPE}{NSPK});
    print "---------------------------------------------\n";
    printf " Face type confusion matrix -- time weighted\n";
    summarize_speaker_type_performance ("TIME", $scores->{TYPE}{TIME});
    print "---------------------------------------------\n";
}
...
#find the mapping that maximizes the cumulative match time between ref and sys spkrs
    if ($opt_b){
    	 $map = weighted_bipartite_graph_match ($cost);
	 }
	 else{
	 $map = no_best_mapping ($spkr_overlap);
	 }
    return $map;
}
...
sub display_metadata_mapping {
    ...
    my %ref_tag = (NOSCORE        => "XS", NO_RT_METADATA => "NM", SEGMENT        => "SG", SPEAKER        => "SP", FACE       =>"SP",
		   SU             => "SU", "A/P"          => "AP", "NON-SPEECH"   => "NS", EDIT           => "ED",
		   FILLER         => "FL", IP             => "IP", CB             => "CB", "NON-LEX"      => "NL",
		   LEXEME         => "LX");
    my %sys_tag = (SPEAKER        => "SP", FACE      =>"SP", SU             => "SU", EDIT           => "ED", FILLER         => "FL",
		   IP             => "IP", LEXEME         => "LX");
...
#compute the costs
    ...
    foreach $ref_spkr (keys %$spkr_overlap) {
	foreach $sys_spkr (keys %{$spkr_overlap->{$ref_spkr}}) {
         $map{$ref_spkr} = $ref_spkr;
	 $map{$sys_spkr} = $sys_spkr;
	}
    }
