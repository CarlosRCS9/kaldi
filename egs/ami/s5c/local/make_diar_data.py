...
def read_rttm(rttm_path, uem_path, df_wav, sep=' ', rttm_suffix=''):
    ...
    # filter rttm based on available wav files
    file_names = df_wav['key'].sort_values().unique()
    rttm = rttm.loc[rttm['file_id'].isin(file_names)].sort_values('file_id')
    ...
    uem = uem.loc[uem['file_id'].isin(file_names)].sort_values('file_id')
...
def make_diar_data(rttm_path, uem_path, wav_path, output_path, partition):
    ...
    print('read rttm')
    rttm = read_rttm(rttm_path, uem_path, df_wav)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--partition', dest='partition', choices=['train', 'dev', 'test'], required=True)
