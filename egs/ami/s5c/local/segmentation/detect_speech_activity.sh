...
if [ $# -ne 5 ]; then
  ...
  echo "  --combine-type <mean|max>  # Method of array combination"
  exit 1
fi
...
test_data_dir=data/mdm/${data_id}${feat_affix}
###############################################################################
...
if [ $stage -le 2 ]; then
  # In this stage we max/average the outputs across all arrays for each
  # session.
  paste -d' ' <(cut -d' ' -f1 $sad_dir/output.scp ) <(cut -d' ' -f1 \
    $sad_dir/output.scp | sed 's/_CH[1-8]//g' ) > $sad_dir/output2session
  utils/utt2spk_to_spk2utt.pl $sad_dir/output2session > $sad_dir/session2output
  matrix-${combine_type} ark:$sad_dir/session2output scp:$sad_dir/output.scp \
    ark,scp:$sad_dir/output_${combine_type}.ark,$sad_dir/output_${combine_type}.scp

  mv $sad_dir/output.scp $sad_dir/output_all.scp
  mv $sad_dir/output_${combine_type}.scp $sad_dir/output.scp
fi
...
if [ $stage -le 3 ]; then
  ...
  # for the new data.
  mkdir -p ${test_data_dir}_${combine_type}
  rm $test_data_dir/utt2dur
  utils/data/get_utt2dur.sh --nj $nj --cmd "$cmd" $test_data_dir || exit 1
  sed 's/_CH[1-8]//g' $test_data_dir/utt2dur > $test_data_dir/utt2dur.bak
  sort -k1,1n -k2,2g $test_data_dir/utt2dur.bak |\
    awk '!a[$1] {a[$1] = $2} $2 == a[$1]' > ${test_data_dir}_${combine_type}/utt2dur

  paste -d' ' <(cut -d' ' -f1 ${test_data_dir}_${combine_type}/utt2dur ) \
    <(cut -d' ' -f1 ${test_data_dir}_${combine_type}/utt2dur ) \
    > ${test_data_dir}_${combine_type}/utt2spk
  cp ${test_data_dir}_${combine_type}/utt2spk ${test_data_dir}_${combine_type}/spk2utt
...
if [ $stage -le 6 ]; then
  local/segmentation/post_process_sad_to_segments.sh \
    ...
    ${test_data_dir}_${combine_type} ${seg_dir} ${seg_dir}
fi

if [ $stage -le 7 ]; then
  local/segmentation/subsegment_data_dir.sh ${test_data_dir}_${combine_type} ${seg_dir}/segments \
    ${data_dir}_${combine_type}_seg
fi
