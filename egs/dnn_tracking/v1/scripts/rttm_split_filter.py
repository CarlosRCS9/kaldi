  ...
  for file_id in sorted(files_segments.keys()):
    ...
    for segment in segments:
      ...
      while True:
        ...
        if new_segment.get_turn_duration() >= args.min_length and len(new_segment.get_speakers()) <= args.max_segment_speakers and all([speaker.get_name() in speakers for speaker in new_segment.get_speakers()]):
