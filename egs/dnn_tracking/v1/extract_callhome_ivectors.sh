...
mfcc_conf=conf/mfcc_callhome_ivectors.conf
extractor_dim=128
extractor_model=../../callhome_diarization/v1/exp/extractor_c2048_i$extractor_dim
...
if [ $stage -le 4 ]; then
  ...
  for name in callhome1 callhome2; do
    ...
    copy-vector \
    scp:$folder/exp/make_ivectors/ivector.scp \
    ark,t:$folder/exp/make_ivectors/ivector.txt
  done
fi
