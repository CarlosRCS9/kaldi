...
if [ $stage -le 6 ]; then
  ...
  $folder2/exp/make_ivectors/plda_scores/rttm | sed --expression='s/ 0 / 1 /g' \
    | md-eval.pl -r \
...
if [ $stage -le 7 ]; then
  ...
  $folder2/exp/make_ivectors/plda_scores_num_spk/rttm | sed --expression='s/ 0 / 1 /g' \
  ...
  cat exp/results/DER_num_spk.txt
fi
